...
makeRTAliases xts = mapM_ expBody xts
  where expBody (mod,xt) = inModule mod $ do
                             ...
                             setRTAlias (rtName xt) $ Right $ mapRTAVars stringRTyVar $ xt { rtBody = body }
...
expandAlias :: SourcePos -> [String] -> BareType -> BareM SpecType
expandAlias l = go
  where
    ...
    go s (RVar a r)       = RVar (stringRTyVar a) <$> resolve l r
...
expandRTApp :: SourcePos -> [String] -> RTAlias RTyVar SpecType  -> [BareType] -> RReft -> BareM SpecType
...
makeClasses cfg vs (mod, spec) = inModule mod $ mapM mkClass $ Ms.classes spec
  where
    ...
    mkClass (RClass c ss as ms)
            = do let l   = loc c  
                 tc  <- lookupGhcTyCon (symbolString <$> c)
