...
instance (Show k, ToJSON a) => ToJSON (Assoc k a) where
  toJSON (Asc kas) = object [ fromShow k .= toJSON a | (k, a) <- M.toList kas ]
    where
...
_anns :: AnnTypes
_anns = assoc [(5,   assoc [( 14, A1 { ident = "foo"
                             , ann   = "int -> int"
                             , row   = 5
                             , col   = 14
                             })
...
assoc :: (Eq k, Hashable k) => [(k, a)] -> Assoc k a
assoc = Asc . M.fromList
