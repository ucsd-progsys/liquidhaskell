...
targetFInfo info cgi fn = F.fi cs ws bs ls gs consts ks {- packs -} qs bi fn aHO aHOqs
  where
    ...
    bs                  = binds    cgi
    ls                  = fEnv     cgi
    gs                  = F.fromListSEnv gs'
    consts              = (F.symbolSymConst . fst) <$> ss'
    ks                  = kuts     cgi
    ...
    (ss', gs')          = partition F.isSymbolSymConst $ F.toListSEnv $ cgConsts cgi 
