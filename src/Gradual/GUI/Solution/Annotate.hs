module Gradual.GUI.Solution.Annotate (renderHtml) where 
...
pretag  :: Int -> Int -> String 
pretag i j 
  = "<div class='dropdown'>" ++ 
    "<span class='dropbtn'>" ++ 
    "<span class='"++ sourceName i++  "' id =" ++ name ++ "'> " 
  where
    name = "'content-" ++  show i ++ "-" ++ show j ++ "'"


posttag :: Int -> Int -> String 
posttag _ _ 
  = "</span>"++ "</span>" ++ "</div>"


tag :: Loc -> [(String, Loc)] -> (Int, Int, SrcSpan, String) -> [(String, Loc)]
tag eof toks (i, j, sp, _) = go False False toks 
  where
    go _ True [] = [(posttag i j , eof)]
    go _ _    [] = []
    go pt b    ((s,l):rest)
      | l `inLoc` sp, not b 
      = (if pt then " " else "" ++ pretag i j ++ s , l):go True True rest 
      | not (l `inLoc` sp), b 
      = (posttag i j ++ s, l):rest 
      | otherwise 
      = (s,l):go False b rest 
...
bform :: Int -> String
bform i = 
  "<button type='button' class='" ++ classbuttonName i ++ 
  "' onclick=\"solvePrev(" ++ show i ++ ")\"><<</button>" ++ 
  "<span type='button' id='button-" ++ show i ++ "' onclick='gradualClick(" ++ show i ++ 
    ")' class='"++ classbuttonName i ++"'>??</span>" ++ 
  "<button type='button' class='" ++ classbuttonName i ++ "' onclick=\"solveNext(" ++ show i 
    ++ ")\">>></button>"
...
htmlHeader initSrc title = unlines
  ...
  , "<script src='http://goto.ucsd.edu/~nvazou/gradual/solve.js'></script>"
  , initSrc
