...
import Prelude hiding (length, map, filter, head, tail)
...
First, lets write a function `keyMap` that expands a list of inputs into a list of key-value pairs:
...
\begin{code}
group     :: [(k, v)] -> M.Map k [v] 
group kvs = foldr (\(k, v) m -> inserts k v m) M.empty vs
\end{code}

The function `inserts` simply adds the new value `v` to the previously
known values `lookupDefault [] k m` for the key `k`.

\begin{code}
inserts :: k -> v -> M.Map k [v] -> M.Map k [v]
inserts k v m = M.add k (v : vs) m 
...
\begin{code}
mapReduce f op = M.toList . reduce op . group . keyMap f
\end{code}
