...
expandRApp tce tyi t@RApp{} = RApp rc' ts rs' r
  where
    ...
    -- Recursively expand RApp nodes inside newly-created RProp bodies.
    -- Without this, bodies generated by rtPropTop/mkRTProp contain
    -- unexpanded RApps (e.g., partially-applied List with empty RProps),
    -- leading to depth mismatches in splitC comparisons (#2692).
    expandRefs                 = map (\(RProp ss body) -> RProp ss (mapBot (expandRApp tce tyi) body))
expandRApp _ _ t               = t
