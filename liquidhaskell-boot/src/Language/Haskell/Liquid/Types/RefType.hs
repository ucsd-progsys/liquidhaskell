...
expandRApp tce tyi t@RApp{} = RApp rc' ts rs' r
  where
    ...
    -- Expand RApp nodes inside newly-created RProp bodies by one extra level.
    -- We use 'expandRAppOnce' (not 'expandRApp') to avoid infinite recursion
    -- on self-referential abstract refinements such as
...
-- RProp slots.  Used by the 'expandRefs' helper inside 'expandRApp' so that
-- the mutual recursion terminates even when a PVar's type is the enclosing
-- type constructor itself (self-referential abstract refinements).
expandRAppOnce :: (PPrint r, IsReft r, SubsTy RTyVar RSort r, Variable r ~ Symbol, ReftBind r ~ Symbol, ReftVar r ~ Symbol, IsReft r, Meet r)
               => TCEmb TyCon -> TyConMap -> RRType r -> RRType r
expandRAppOnce tce tyi t@RApp{} = RApp rc' ts rs' r
  where
    RApp rc ts rs r            = t
    (rc', _)                   = appRTyCon tce tyi rc as
    pvs                        = rTyConPVs rc'
    rs'                        = applyNonNull rs0 (rtPropPV rc pvs) rs   -- no expandRefs
    rs0                        = rtPropTop <$> pvs
    n                          = length fVs
    fVs                        = GM.tyConTyVarsDef $ rtc_tc rc
    as                         = choosen n ts (rVar <$> fVs)
expandRAppOnce _ _ t           = t
