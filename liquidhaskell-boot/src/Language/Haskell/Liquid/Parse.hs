...
bTyConP
  =  (reservedOp "'" >> (mkPromotedBTyCon . fmap (makeUnresolvedLHName LHTcName) <$> locUpperIdP))
...
bTup ts rs r
  ...
  = RApp (mkBTyCon $ dummyLoc $ makeUnresolvedLHName LHTcName tupConName) (snd <$> ts) rs (reftUReft r)
  | otherwise
  = RApp
      (mkBTyCon $ dummyLoc $ makeUnresolvedLHName LHTcName tupConName)
      (top . snd <$> ts)
      rs'
      (reftUReft r)
  where
