...
def csvify(fn, metrics):
    return '%s, %d, %d, %d, %d, %d\n' % (
        fn, metrics['files'], metrics['sloc'], metrics['specs_lines'] +
        metrics['others_lines'], int(metrics['timeL']),
        int(metrics['timeF']))
...
def main():
    ...
    for d, fs in benchmarks.iteritems():
        ...
        for fn in fs:
            ...
            f_res['timeF'] = timeDictF["Tests/Benchmarks/%s/%s" % (benchmarksNames[d], fn)]
            f_res['timeL'] = timeDictL["Tests/Benchmarks/%s/%s" % (benchmarksNames[d], fn)]
    ...
    with open('metrics.csv', 'w') as out:
        out.write("Benchmark, Files, Code, Spec, Time (L), Time (F)\n")
