...
hedgeUnion :: Ord a => Map a b -> Map a b -> Map a b
hedgeUnion t1  Tip = t1
hedgeUnion Tip (Bin _ kx x l r) = undefined -- join kx x (filterGt blo l) (filterLt bhi r)
hedgeUnion t1  (Bin _ kx x Tip Tip) = undefined -- insertR kx x t1 -- According to benchmarks, this special case increases
                                                              -- performance up to 30%. It does not help in difference or intersection.
hedgeUnion t1@(Bin _ kx x l r) t2@(Bin _ _ _ _ _) = undefined -- join kx x (hedgeUnion blo bmi l (trim blo bmi t2))
hedgeUnion t1 t2 = undefined -- join kx x (hedgeUnion blo bmi l (trim blo bmi t2))
