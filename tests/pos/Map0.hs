...
singleL k1 x1 t1 (Bin _ k2 x2 t2s1 t3) = bin k2 x2 (bin k1 x1 t1 t2s1) t3
singleL _  _  _ Tip                  = error "sinlgeL Tip"
singleR k1 x1 (Bin _ k2 x2 t1 t2s2) t3 = Bin 0 k2 x2 t1 (Bin 0 k1 x1 t2s2 t3)
singleR _  _  Tip _                  = error "sinlgeR Tip"

doubleL, doubleR :: a -> b -> Map a b -> Map a b -> Map a b
doubleL k1 x1 t1 (Bin _ k2 x2 (Bin _ k3 x3 t2d1 t3) t4)
 =bin k3 x3 (bin k1 x1 t1 t2d1) (bin k2 x2 t3 t4)
doubleL _ _ _ _ = error "doubleL" 
doubleR k1 x1 (Bin _ k2 x2 t1d2 (Bin _ k3 x3 t2d2 t3)) t4 
  = bin k3 x3 (bin k2 x2 t1d2 t2d2) (bin k1 x1 t3 t4)
doubleR _ _ _ _ = error "doubleR" 

bin :: k -> a -> Map k a -> Map k a -> Map k a
bin k x l r 
  = undefined -- Bin (size l + size r + 1) k x l r
