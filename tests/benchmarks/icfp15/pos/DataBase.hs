...
{-@ dfun :: forall <range :: key -> val -> Bool>.
            ...
            -> i:{v:key | Set_mem v (Set.listElts (ddom x))} -> val<range i>
  @-}

{-@ data Dict key val <range :: key -> val -> Bool>
      = D ( ddom :: [key])
          ( dfun :: i:{v:key | Set_mem v (Set.listElts ddom)} -> val<range i>)
  @-}
...
{-@ union :: forall <range :: key -> val -> Bool, p :: Dict key val -> Bool>.
          ...
          -> {v:[Dict <range> key val <<p>>] | Set.listElts v = Set_cup (Set.listElts x) (Set.listElts y)}
  @-}
{-@ diff :: forall <range :: key -> val -> Bool, p :: Dict key val -> Bool>.
              x:[Dict <range> key val <<p>>]
          ->  y:[Dict <range> key val <<p>>]
          -> {v:[Dict <range> key val <<p>>] | Set.listElts v = Set_dif (Set.listElts x) (Set.listElts y)}
  @-}
union, diff :: (Eq key, Eq val) => Table key val -> Table key val -> Table key val
union xs ys = xs ++ ys
diff  xs ys = xs \\ ys

{-@ predicate Append XS YS V = (Set.listElts (ddom V)) = Set_cup (Set.listElts (ddom YS)) (Set.listElts (ddom XS)) @-}
...
{-@ product :: forall <range1  :: key -> val -> Bool,
                       ...
                       q :: Dict key val -> Bool,
                       r :: Dict key val -> Bool>.
                       {x::Dict key val <<p>>, y :: Dict key val <<q>> |- {v:Set.Set key |  v = Set_cap (Set.listElts (ddom x)) (Set.listElts (ddom y))}  <: {v:Set.Set key | Set_emp v }}
                       {x::Dict key val <<p>>, y :: Dict key val <<q>> |-  {v:Dict key val | Append x y v} <: Dict key val <<r>>}
                       {x::Dict key val <<p>>, k::{v:key | Set_mem v (Set.listElts (ddom x))} |- val<range1 k> <: val<range k> }
                       {x::Dict key val <<q>>, k::{v:key | Set_mem v (Set.listElts (ddom x))} |- val<range2 k> <: val<range k> }
...
{-@ productD :: forall <range1  :: key -> val -> Bool,
            ...
            -> {v:Dict <range> key val | (Set.listElts (ddom v)) = Set_cup (Set.listElts (ddom x)) (Set.listElts (ddom y))}
  @-}
...
{-@ project :: forall <range :: key -> val -> Bool>.
               keys:[key]
            -> [{v:Dict <range> key val | (Set_sub (Set.listElts keys) (Set.listElts (ddom v)))}]
            -> [{v:Dict <range> key val  | (Set.listElts (ddom v)) = Set.listElts keys}]
   @-}
...
{-@ select :: forall <range :: key -> val -> Bool, p :: Dict key val -> Bool>.
          ...
          -> x:[Dict <range> key val <<p>>]
          -> {v:[Dict <range> key val <<p>>] | Set_sub (Set.listElts v) (Set.listElts x)}
  @-}
select :: (Dict key val -> Bool) -> Table key val -> Table key val
select prop xs = filter prop xs

{-@ values :: forall <range :: key -> val -> Bool>.
      k:key -> [{v:Dict <range> key val | Set_mem k (Set.listElts (ddom v))}]  -> [val<range k>] @-}
values :: key -> [Dict key val]  -> [val]
values k = map (goValues k)

goValues :: key -> Dict key val -> val
{-@ goValues :: forall <rr :: key -> val -> Bool>.
              k:key -> {v:Dict <rr> key val | Set_mem k (Set.listElts (ddom v))} -> val<rr k>  @-}
goValues k (D _ f) = f k


{-@ empty :: {v:Dict <{\k v -> false}> key val | Set_emp (Set.listElts (ddom v))} @-}
empty :: Dict key val
...
{-@ extend :: forall <range :: key -> val -> Bool>.
              k:key-> val<range k>
           -> x:Dict <range> key val
           -> {v:Dict <range> key val | (Set.listElts (ddom v)) = (Set_cup (Set.listElts (ddom x)) (Set_sng k))} @-}
...
{-@ += :: forall <range :: key -> val -> Bool>.
              pp:P <range> key val
           -> x:Dict <range> key val
           -> {v:Dict <range> key val | (Set.listElts (ddom v)) = (Set_cup (Set.listElts (ddom x)) (Set_sng (kkey pp)))} @-}
...
{-@ ensuredomain :: forall <p ::a -> Bool>. Eq a => xs:[a<p>] -> x:{v:a | Set_mem v (Set.listElts xs)} -> {v:a<p> | Set_mem v (Set.listElts xs) && v = x} @-}
...
{-@ (\\) :: forall<p :: a -> Bool>. xs:[a<p>] -> ys:[a] -> {v:[a<p>] | (Set.listElts v)  = (Set_dif (Set.listElts xs) (Set.listElts ys))} @-}
(\\) :: Eq a => [a] -> [a] -> [a]
[]     \\ _ = []
(x:xs) \\ ys = if x `elem` ys then xs \\ ys else x:(xs \\ ys)


{-@ assume (++) :: xs:[a] -> ys:[a] -> {v:[a] | Set.listElts v = Set_cup (Set.listElts xs) (Set.listElts ys)} @-}

{-@ assume elem :: x:a -> xs:[a] -> {v:Bool | v <=> Set_mem x (Set.listElts xs)} @-}
elem :: a -> [a] -> Bool
elem = undefined

{-@ filter :: (a -> Bool) -> xs:[a] -> {v:[a] | Set_sub (Set.listElts v) (Set.listElts xs)} @-}
filter :: (a -> Bool) -> [a] -> [a]
filter _ []   = []
filter f (x:xs)
  | f x       = x : filter f xs
...
{-@ qual :: xs:[a] -> {v: a | Set_mem v (Set.listElts xs)} @-}
qual :: [a] -> a
qual = undefined
...
{-@ qual1 :: ks:[key] -> {v:Dict key val | (Set.listElts (ddom v)) = Set.listElts ks} @-}
