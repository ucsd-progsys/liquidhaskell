...
-- coming from distint imports using the name qualified.
--
-- Here, both @Nat@ modules export a type alias with
-- the same name @INat@. The appropiate alias is used in each case.
-- This example show that we can use both the unqualified or qualified name
...
-- accoding to lexicographic order. So here, @INat@ from 'Nat2' is exported by
-- this module.
