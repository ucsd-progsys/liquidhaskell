...
The github repo builds with GHC `9.14.1`, cabal-install `3.16.1.0`, and `z3` `4.15.1` or newer, all of
...
When `liquidhaskell` tests run, we can collect measures with

    $ ./scripts/test/test_plugin.sh --measure-timings

Measures will be collected in `.dump-timings` files under `dist-newstyle` directory.
This runs all tests and measures in parallel. The `--measure-timings` flag is reliable
if you want to measure allocations. But if you want to measure time, and you want to
reduce interference between the tests, you need to use the flag `--measure-timings-j1`
instead, which runs tests one at a time.

The `.dump-timings` files can be converted to json data with
...
Use `--phase LiquidHaskell` to get the wall-clock time instead.

Comparison charts in `svg` format can be generated by invoking

```
cabal run plot-performance -- -b path_to_before_summary.csv -a path_to_after_summary.csv -s 50 --allocs
```

This will generate two files `top.svg` and `bot.svg` containing the top 50 improvements and
regressions over the entire test set, both enabled by the `-s` option) in the current
directory. If you want to see time, instead of allocations, drop the `--allocs` flag.

If not all the tests are relevant, a subset of the tests can be selected by a prefix of their
names with `-f <prefix>`, for instance
