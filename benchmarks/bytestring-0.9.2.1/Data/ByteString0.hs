...
split w (PS x s l) = inlinePerformIO $ withForeignPtr x $ \p -> do
    let ptr = p `plusPtr` s
        ...
        STRICT1(loop)
        loop n =
            -- LIQUID: else lose `plen` information due to subsequent @ Word8 application
...
--     return (sloop xanadu ptrGOBBLE w l s 0)


{- sloop :: fp:(ForeignPtr Word8) 
          -> p:(Ptr Word8) 
          -> Word8 
          -> l:{v:Nat | v <= (plen p)} 
  ...
  @-}
sloop :: ForeignPtr Word8 -> Ptr Word8 -> Word8 -> Int -> Int -> Int -> [ByteString]
sloop xanadu ptrGOBBLE w l s n = 
  ...
  in if isNullPtr q {- LIQUID q == nullPtr -}
       then [PS xanadu (s+n) (l-n)]
            ...
            in PS xanadu (s+n) (i-n) : sloop xanadu ptrGOBBLE w l s (i+1)
